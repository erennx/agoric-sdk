...
async function makeWorker() {
  ...
  await vat.evaluate(`
    ...
    const fromArrayBuffer = String.fromArrayBuffer;
    const fromString = ArrayBuffer.fromString;
    globalThis.handleCommand = bytes => {
      const report = {};
      const src = fromArrayBuffer(bytes);
      const it = eval(src);
      const output = printed;
      printed = [];
      output.push(it);
      report.result = fromString(JSON.stringify(output));
      return report;
    };
  `);
